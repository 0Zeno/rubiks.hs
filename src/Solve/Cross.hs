...
insertFromD (EdgeCubie _ Neutral) DF = [F, F]
insertFromD (EdgeCubie _ Flipped) DF = [F, U, Lp, Up]
insertFromD (EdgeCubie _ Neutral) DR = [R, R]
insertFromD (EdgeCubie _ Flipped) DR = [R, U, Fp, Up]
insertFromD (EdgeCubie _ Neutral) DL = [L, L]
insertFromD (EdgeCubie _ Flipped) DL = [Lp, Up, F, U]
insertFromD (EdgeCubie _ Neutral) DB = [B, B]
...
solveUF cube@(Cube _ edges) =
    ...
    in case pos of
        UF -> if getEdge edges UF == EdgeCubie UF Neutral
            then cube
            else solveUF (applyMoveList cube [F, F])
        DF -> applyMoveList cube (insertFromD (getEdge edges pos ) DF)
        DR -> solveUF (applyMoveList cube [Dp])
        DL -> solveUF (applyMoveList cube [D])
        DB -> solveUF (applyMoveList cube [D, D])
        UR -> solveUF (applyMoveList cube [R, R])
        UL -> solveUF (applyMoveList cube [L, L])
        UB -> solveUF (applyMoveList cube [B, B])
...
solveUR cube@(Cube _ edges) =
    ...
    in case pos of
        UR -> if getEdge edges UR == EdgeCubie UR Neutral
            then cube
            else solveUR (applyMoveList cube [R, R])
        DR -> applyMoveList cube (insertFromD (getEdge edges pos) DR)
        DF -> solveUR (applyMoveList cube [D])
        DL -> solveUR (applyMoveList cube [D, D])
        DB -> solveUR (applyMoveList cube [Dp])
        UF -> solveUR (applyMoveList cube [F, F])
        UL -> solveUR (applyMoveList cube [L, L])
        UB -> solveUR (applyMoveList cube [B, B])
...
solveUL cube@(Cube _ edges) =
    ...
    in case pos of
        UL -> if getEdge edges UL == EdgeCubie UL Neutral
            then cube
            else solveUL (applyMoveList cube [L,L])
        DL -> applyMoveList cube (insertFromD (getEdge edges pos) DL)
        DF -> solveUL (applyMoveList cube [Dp])
        DR -> solveUL (applyMoveList cube [D,D])
        DB -> solveUL (applyMoveList cube [D])
        UF -> solveUL (applyMoveList cube [F,F])
        UR -> solveUL (applyMoveList cube [R,R])
        UB -> solveUL (applyMoveList cube [B,B])
...
solveUB cube@(Cube _ edges) =
    ...
    in case pos of
        UB -> if getEdge edges UB == EdgeCubie UB Neutral
            then cube
            else solveUB (applyMoveList cube [B,B])
        DB -> applyMoveList cube (insertFromD (getEdge edges pos) DB)
        DF -> solveUB (applyMoveList cube [D,D])
        DR -> solveUB (applyMoveList cube [D])
        DL -> solveUB (applyMoveList cube [Dp])
        UF -> solveUB (applyMoveList cube [F,F])
        UR -> solveUB (applyMoveList cube [R,R])
        UL -> solveUB (applyMoveList cube [L,L])
